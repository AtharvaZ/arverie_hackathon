 ...
 */
export function computeSnapshot(behaviorData, canvasWidth, canvasHeight, sessionStartMs) {
  const now = Date.now()
  ...
  const recentColorChanges = colorChanges.filter((c) => c.timestamp >= windowStart)
  const colorsInWindow = Array.from(
    new Set([
      ...(recentColorChanges.length > 0 ? recentColorChanges.map((c) => c.to) : []),
      currentColor,
    ])
  )
  ...
  if (recentStrokes.length > 0 && canvasWidth > 0 && canvasHeight > 0) {
    // DrawingCanvas.onStroke gives no x/y position — distribute strokes uniformly
    const perQuadrant = recentStrokes.length / 4
    for (const k of Object.keys(quadrantCounts)) {
      quadrantCounts[k] = perQuadrant
    }
  }

  // Use erase events (which DO have position) to refine quadrant distribution
  const positionedEvents = [
    ...recentErasures.map((e) => ({ x: e.position?.x ?? 0, y: e.position?.y ?? 0 })),
  ]
 ...
 */
export function buildCanvasSummary(behaviorData, canvasWidth, canvasHeight, sessionStartMs) {
  ...
  const allColors = Array.from(
    new Set([...colorChanges.map((c) => c.to), ...strokes.map((s) => s.color).filter(Boolean)])
  )
  ...
  if (erasures.length > 0 && canvasWidth > 0 && canvasHeight > 0) {
    const qCount = { top_left: 0, top_right: 0, bottom_left: 0, bottom_right: 0 }
    for (const e of erasures) {
      const x = e.position?.x ?? 0
      const y = e.position?.y ?? 0
  ...
  // Dominant area
  const dominantArea = Object.entries(timeInQuadrants).reduce((a, b) => (b[1] > a[1] ? b : a))[0]
