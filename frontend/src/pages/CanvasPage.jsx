...
import { computeSnapshot, buildCanvasSummary } from '../hooks/useSnapshotMetrics'
    ...
    <div style={{ position: 'relative', display: 'flex', height: '100%' }}>
      ...
      <button
        ...
        style={{
          ...
          justifyContent: 'center', zIndex: 20, marginLeft: '-28px',
        }}
      >
        <svg width="7" height="12" viewBox="0 0 7 12" fill="none"
          style={{ transform: open ? 'rotate(0)' : 'rotate(180deg)', transition: 'transform 0.3s' }}
        >
          <path d="M5 1 L1 6 L5 11" stroke="#c8a828" strokeWidth="1.4"
      ...
      <motion.div
        ...
        style={{
          height: '100%', background: 'rgba(245,235,208,0.97)',
          ...
          {open && (
            <motion.div
              ...
              style={{
                ...
                padding: '14px', overflow: 'hidden', minWidth: '300px',
              }}
            >
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '4px' }}>
                <OrnamentalDivider label="reflections" style={{ flex: 1, margin: '4px 0' }} />
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    background: isSpeaking ? '#c8a040' : isConnected ? '#5a8a5a' : '#888',
                    transition: 'background 0.3s',
                    boxShadow: isConnected ? '0 0 4px currentColor' : 'none',
                  }} />
                  <span style={{
                    fontFamily: 'Cinzel, serif', fontSize: '9px',
                    letterSpacing: '0.08em', color: 'rgba(80,55,30,0.5)',
                  }}>
              ...
              <div style={{
                ...
                {messages.length === 0 ? (
                  <p style={{
                    ...
                    color: 'rgba(80,55,30,0.45)', textAlign: 'center',
                  ...
                  messages.map((msg, i) => (
                    ...
                    >
                      <p style={{
                        ...
                        color: msg.role === 'assistant' ? 'rgba(80,55,30,0.85)' : 'rgba(80,55,30,0.6)',
                      }}>
                        {msg.content}
                      </p>
                      {msg.timestamp && (
                        <p style={{
                          fontFamily: 'Cinzel, serif', fontSize: '9px',
                          color: 'rgba(80,55,30,0.3)', marginTop: '4px',
                        }}>
                          {msg.timestamp}
                        </p>
                      )}
...
export default function CanvasPage() {
  ...
  const { isConnected, isSpeaking, connect: connectHume, disconnect: disconnectHume } = useHumeVoice({
    ...
    onAIMessage: useCallback((text) => {
      const elapsed = (Date.now() - sessionStartRef.current) / 1000
      const mm = String(Math.floor(elapsed / 60)).padStart(1, '0')
      const ss = String(Math.floor(elapsed % 60)).padStart(2, '0')
      addMessage({ role: 'assistant', content: text, timestamp: `${mm}:${ss}` })
    }, [addMessage]),
  })
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      const snapshot = computeSnapshot(
        behaviorData.current, w, h, sessionStartRef.current
      )
  ...
  useEffect(() => {
    ...
    behaviorData.current.brushChanges.push({ from: prevBrush.current, to: brush, timestamp: Date.now() })
  ...
  async function handleFinish() {
    ...
    const w = canvasEl?.offsetWidth || 0
    const h = canvasEl?.offsetHeight || 0
    const canvasSummary = buildCanvasSummary(behaviorData.current, w, h, sessionStartRef.current)
    ...
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      >
        <button
          ...
          disabled={finishing}
          style={{ padding: '6px 18px', fontSize: '13px', opacity: finishing ? 0.6 : 1 }}
        >
