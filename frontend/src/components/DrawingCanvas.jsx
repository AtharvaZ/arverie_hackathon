...
const DrawingCanvas = forwardRef(function DrawingCanvas(
  ...
  const handlePointerMove = useCallback((e) => {
    ...
    e.preventDefault()
    const target = LIVE_BRUSHES.has(brush) ? liveCanvasRef.current : canvasRef.current
    ...
    switch (brush) {
      ...
      case 'pencil':
        ...
        drawPencil(ctx, fromMid.x, fromMid.y, prev.x, prev.y, currentMid.x, currentMid.y, color, size, 1)
        break
      case 'ink':
        drawInk(ctx, fromMid.x, fromMid.y, prev.x, prev.y, currentMid.x, currentMid.y, color, size, 1, speed)
        break
      case 'smudge':
      case 'blur': {
  ...
  const handlePointerUp = useCallback(() => {
    ...
    if (pts.length > 1) {
      ...
      onStroke?.({
        ...
        opacity,
        size,
        areaCovered: (Math.max(...xs) - Math.min(...xs)) * (Math.max(...ys) - Math.min(...ys)),
      })
    }
